...
- **Per-tenant catalog auto-clone** — `app/Observers/SchoolObserver.php` clones the default school's catalogs onto every newly-created `School` in a single transaction: the flat ones (`pas_allowances`, `pas_deduction_types`) plus the relational ones (`pas_chart_of_accounts`, `pas_tax_rates`), whose intra-set foreign keys (`parent_id`, `account_id`) are remapped by code so a tenant never points at another tenant's rows. Idempotent (skips tables where the new school already has rows). Bypasses Eloquent via raw DB inserts so cloned rows land on the *new* school's id, not `Tenant::current()`.
...
### Database
- MySQL. The two databases are physically split in dev: app-owned `pas_*` tables live in `DB_DATABASE` (`payroll_acct_db`), and the read-only LMS tables in `LMS_DB_DATABASE` (`payroll_db`) on the `lms` connection. Check `.env` before assuming — the split is env-driven, and the connections were originally configured to mirror one database
